...
무결성 : 허가받은 사용자 중에 권한이 잘못 부여되어 시스템(데이터)에 문제를 일으키는 행위

무결성을 보장하기 위하여
생성된 사용자에게 적절한 권한을 제한적으로 부여하여 여러 민감한 데이터의 손상, 
변화(INSERT UPDATE DELETE)를 최소화 하여야한다
*/

-- necktie 사용자에게  bookDB2 데이터베이스에 대한 권한을 부여
-- TABLE 생성, CRUD를 수행할 수 있다. 
GRANT ALL PRIVILEGES ON bookDB2.* TO 'necktie'@'localhost';

-- 기존에 부여된 권한 회수
REVOKE ALL PRIVILEGES ON *.* FROM 'necktie'@'localhost';

USE bookdb2;
DROP TABLE tbl_bookstbl_bbd;